...
def synthesize_speech_bytes(*, text: str, voice: str, speed: float) -> SpeechSynthesisResult:
    text = text.strip()
    if not text:
    ...
    if provider == "openai":
        return _synthesize_with_openai(text=text, voice=voice, speed=speed)
...
def _synthesize_with_openai(*, text: str, voice: str, speed: float) -> SpeechSynthesisResult:
    settings = get_settings()
    client = OpenAI(api_key=settings.openai_api_key)
    try:
