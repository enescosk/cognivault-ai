...
from app.models import (
    Clinic, ClinicChannel, ClinicConversation, ClinicConversationStatus, ClinicDoctor, ClinicDoctorSlot, ClinicPatient,
...
def create_clinical_appointment_from_conversation(
    ...
    clinic_doctor: ClinicDoctor | None = None
    ...
    if slot_id is not None:
        ...
        duration_minutes = max(15, int(((slot.end_time - slot.start_time).total_seconds() // 60) or duration_minutes))

    if doctor_id is not None:
        clinic_doctor = db.get(ClinicDoctor, doctor_id)
        if clinic_doctor is None or clinic_doctor.clinic_id != clinic.id:
            raise NotFoundError("Doctor not found")
        if not department or department == "Muayene":
            department = clinic_doctor.specialty

    appointment = ClinicalAppointment(
        clinic_id=clinic.id,
        patient_id=conversation.patient_id,
        conversation_id=conversation.id,
        doctor_id=clinic_doctor.id if clinic_doctor else None,
        slot_id=slot_id,
